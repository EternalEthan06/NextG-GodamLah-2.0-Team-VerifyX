...
def verify_face():
    ...
    try:
        ...
        # --- SPOOF CHECK (Texture Analysis) ---
        score, is_spoof = analyze_texture(img)
        if is_spoof:
             log_action(mykad, "VERIFY", f"Spoof/Blur Detected (Score: {score:.2f})", "System", "FAILED")
             return jsonify({'status': 'failure', 'message': 'Anti-Spoofing Check Failed. Image too blurry or artificial.'})
        
        # Get encoding (assume 1 face per image for security)
        ...
        if len(encodings) > 1:
             return jsonify({'status': 'failure', 'message': 'Multiple faces detected. Ensure only you are in frame.'})
             
        new_encoding = encodings[0]
        
        # Read the file stream again to store the blob
        file.seek(0)
        image_blob = file.read()
        
        # Serialization
        new_encoding_blob = pickle.dumps(new_encoding)

        # 1. ENROLLMENT (First time) - Check global flag or session
        # 1. ENROLLMENT (First time) - Check session
        if not session.get('is_enrolled') and session.get('enrollment_stage') == 'face':
            ...
            log_action(mykad, "ENROLL", "Face Biometric Registered", "System", "SUCCESS")
            return jsonify({'status': 'success', 'message': 'Face registered successfully.'})

        # 2. VERIFICATION (Returning)
        else:
            conn = get_db_connection()
            ...
            stored_encoding = pickle.loads(user_record['face_encoding_blob'])
            
            # Compare
            # tolerance=0.45 for stricter security
            matches = face_recognition.compare_faces([stored_encoding], new_encoding, tolerance=0.45)
...
def check_blink():
    ...
    try:
        ...
        # Get Face Landmarks
        # face_landmarks method returns a list of dicts (one for each face)
        landmarks_list = face_recognition.face_landmarks(img)
        
        if not landmarks_list:
             return jsonify({'blink': False, 'message': 'No face'})
             
        landmarks = landmarks_list[0]
        
        # Extract eye coordinates (list of tuples)
        ...
        # Blink Threshold (Typical is 0.2 - 0.3)
        BLINK_THRESHOLD = 0.25
        
        if ear < BLINK_THRESHOLD:
             return jsonify({'blink': True, 'ear': ear})
        else:
             return jsonify({'blink': False, 'ear': ear})

    except Exception as e:
        print(f"Blink Check Error: {e}")
        return jsonify({'blink': False, 'error': str(e)})
...
def check_pose():
    ...
    try:
        ...
        if not landmarks_list:
             print("DEBUG: Pose Check - No Face Detected")
             return jsonify({'direction': 'none'})
             
        direction = get_head_pose(landmarks_list[0])
        print(f"DEBUG: Detected Direction: {direction}")
...
def verify_gesture_identity():
    ...
    try:
        ...
        stored_encoding = pickle.loads(user_record['face_encoding_blob'])
        matches = face_recognition.compare_faces([stored_encoding], new_encoding, tolerance=0.5) # 0.5 strictness
        
        if not matches[0]:
             log_action(mykad, "GESTURE_VERIFY", "Face Mismatch during Gesture", "System", "FAILED")
             return jsonify({'status': 'failure', 'message': 'Identity verification failed. Face does not match.'})

        # 4. BLINK / EYE ANALYSIS (Requested by User)
        # We calculate EAR. 
        # Note: A single frame cannot prove "Liveness" via blinking movement, 
        # but we can verify eyes are present and open (or closed).
        ...
        if landmarks_list:
            ...
            leftEAR = eye_aspect_ratio(left_eye)
            rightEAR = eye_aspect_ratio(right_eye)
            ear_score = (leftEAR + rightEAR) / 2.0
            
            # If EAR < 0.2, eyes are likely closed (Blink)
            if ear_score < 0.2:
                blink_status = "Eyes Closed (Blink Detected)"
            else:
                blink_status = "Eyes Open"
                
        # Log and Return Success
        print(f"DEBUG: Gesture Identity Verified. EAR: {ear_score:.2f} ({blink_status})")
        log_action(mykad, "GESTURE_VERIFY", f"Identity Confirmed + {blink_status}", "System", "SUCCESS")
        
        return jsonify({'status': 'success', 'message': 'Identity confirmed.', 'blink_info': blink_status})
